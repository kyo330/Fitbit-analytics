...
# Styles
# Light theme: dark text on warm off-white backgrounds.
# Key rules: body text #1a1a2e, secondary #4a4760, muted #6b6880.
st.markdown("""
<style>
...
/* ── Page title (serif + gradient) ── */
.page-title {
    font-family: 'DM Serif Display', serif;
    font-size: 2rem;
    font-weight: 400;
    background: linear-gradient(120deg, #e05c2a, #c0392b, #8b2fc9);
...
/* ── Sidebar: one shade darker than the main background ── */
.stSidebar { background-color: #ede9e3 !important; }
section[data-testid="stSidebar"] { background-color: #ede9e3 !important; }
section[data-testid="stSidebar"] * { color: #1a1a2e !important; }
section[data-testid="stSidebar"] .stRadio label { color: #1a1a2e !important; }
section[data-testid="stSidebar"] .stSelectbox label { color: #1a1a2e !important; }
section[data-testid="stSidebar"] p { color: #4a4760 !important; }
section[data-testid="stSidebar"] small { color: #6b6880 !important; }

/* ── Dataframes ── */
.stDataFrame, .stDataFrame * { color: #1a1a2e !important; }

div[data-testid="stMetricValue"] { color: #e05c2a; font-weight: 700; }
</style>
""", unsafe_allow_html=True)


# Color palette used across all charts
PALETTE = ["#e05c2a", "#8b2fc9", "#27ae60", "#2980b9", "#e07b2a", "#c0392b", "#6c5ce7"]
...
CHART_DEFAULTS = dict(
    ...
    legend=dict(bgcolor="rgba(0,0,0,0)", font_color="#1a1a2e"),
    xaxis=dict(gridcolor="#e8e3dc", showgrid=True, zeroline=False, color="#4a4760"),
    yaxis=dict(gridcolor="#e8e3dc", showgrid=True, zeroline=False, color="#4a4760"),
...
def load_data():
    """
    Load Customers.csv from the same folder as this script.
    Column names vary across Kaggle versions so we normalize them here.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    search_paths = os.path.join(base_dir, "Customers.csv"),
...
def kpi_card(value, label, sub="", color="#2980b9"):
    st.markdown(f"""
    <div class="kpi-card">
...
def show_overview(df):
    ...
    cols = st.columns(4)
    with cols[0]:
        kpi_card(f"{len(df):,}", "Total Customers", "in dataset", "#2980b9")
    with cols[1]:
        kpi_card(
    ...
    if "Gender" in df.columns:
        ...
        with row2[0]:
            kpi_card(f"{female_pct:.0f}% F / {100 - female_pct:.0f}% M", "Gender Split", "", "#e05c2a")
    if "Profession" in df.columns:
        with row2[1]:
            kpi_card(f"{df['Profession'].nunique()}", "Professions", "distinct categories", "#2980b9")
    if "FamilySize" in df.columns:
        with row2[2]:
            kpi_card(f"{df['FamilySize'].mean():.1f}", "Avg Family Size", "members per household", "#2980b9")
    if "WorkExperience" in df.columns:
        with row2[3]:
            kpi_card(f"{df['WorkExperience'].mean():.1f} yrs", "Avg Work Experience", "", "#27ae60")
...
def show_demographics(df):
    ...
    with col1:
        fig = px.histogram(df, x="Age", nbins=25, color_discrete_sequence=["#e05c2a"])
        apply_chart_style(fig, "How old are our customers?", 320)
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        fig = px.histogram(df, x="AnnualIncome", nbins=25, color_discrete_sequence=["#27ae60"],
...
def show_relationships(df):
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorbar=dict(thickness=14, tickfont_color="#1a1a2e"),
    ))
    apply_chart_style(fig, "Pearson Correlation Between Key Variables", 460)
    st.plotly_chart(fig, use_container_width=True)

    section("Key Pairwise Relationships")
    st.markdown('<p style="color:#6b6880;font-size:0.88rem;margin-top:-12px;margin-bottom:16px;">'
                'Each chart shows the trend line and Pearson r value.</p>', unsafe_allow_html=True)

    pairs = [
        ("AnnualIncome", "SpendingScore", "#e05c2a", "Does earning more mean spending more?"),
        ("Age", "SpendingScore", "#27ae60", "Do older customers spend less?"),
        ("Age", "AnnualIncome", "#2980b9", "Does income rise with age?"),
    ]
    ...
    if "FamilySize" in df.columns:
        ...
        fig = go.Figure(go.Scatter(
            ...
            mode="lines+markers",
            line=dict(color="#8b2fc9", width=2.5),
            marker=dict(size=9, color="#8b2fc9"),
        ))
...
def show_insights(df):
    ...
    st.markdown(
        '<p style="color:#6b6880;font-size:0.88rem;margin-top:-12px;margin-bottom:20px;">'
...
def main():
    ...
    if df is None:
        st.error(
            "Could not find Customers.csv in this folder. "
            "Make sure Customers.csv sits in the same folder as customer_intelligence_app.py. "
            "Download it from https://www.kaggle.com/datasets/datascientistanna/customers-dataset"
        )
        st.stop()
